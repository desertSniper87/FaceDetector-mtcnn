...
class iCartoonFace(object):

    def __init__(self, dataset_folder=os.path.join(here, 'iCartoonFace')):
        """
        Make sure the iCartoonFace dataset saved in $SOURCE_ROOT/datasets/iCartoonFace folder.
        """
        self.dataset_folder = dataset_folder

    def _load_meta_data(self, meta_file, target_folder=''):
        """Load metadata of iCartoonFace dataset.

        Args:
            meta_file (str): E.g. iCartoonFace/wider_face_split
            target_folder (str): E.g. iCartoonFace/WIDER_train/images
    ...
    def get_train_meta(self):
        meta_file = os.path.join(self.dataset_folder, "icartoonface_dettrain.csv")
        target_folder = os.path.join(self.dataset_folder, 'icartoonface_dettrain')

        return self._load_meta_data(meta_file, target_folder)

    def get_val_meta(self):
        meta_file = os.path.join(self.dataset_folder, "icartoonface_detval.csv")
        target_folder = os.path.join(self.dataset_folder, 'icartoonface_detval')
    ...
    def get_test_meta(self):
        ...
        """
        meta_file = os.path.join(self.dataset_folder, 'icartoonface_dettest.csv')
        target_folder = os.path.join(self.dataset_folder, 'icartoonface_dettrain')
        f = open(meta_file)
        ret = list()
